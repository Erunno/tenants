...
export default function TenantRow({ tenantData, onEdit, onDelete, onToggleHidden }) {
    return (
        <tr className={`${tenantData.hidden ? 'hidden-tenant-row' : ''} ${tenantData.newlyAdded ? 'newly-added-tenant-row' : ''}`}>
            ...
            <td><div>{tenantData.to ? tenantData.to.format(showDateFormat) : '...'}</div></td>
            <td><div>{countDays(tenantData.from, tenantData.to)}</div></td>
            <td><div className="btns-container">
                <span className="edit-btn me-1 mt-auto mb-auto" onClick={onEdit}>
